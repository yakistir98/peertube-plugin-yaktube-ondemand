  ...
  (function () {
        ...
        if (!alt || alt.trim() === '') {
          ...
          if (card) {
            var titleEl = card.querySelector(
              '.video-miniature-name, .video-name, .yaktube-video-name, h3, h4, a[title]'
            );
    ...
    function transformToYouTubeSearchBar(searchInp, searchContainer) {
      ...
      nativeInputs.forEach(function (el) {
        if (
          !el.classList.contains('yaktube-search-trigger-pill') &&
          !el.classList.contains('yaktube-voice-search-btn')
        ) {
          el.style.display = 'none';
    ...
    function checkAndInjectRelatedVideos(forceReload) {
      var isWatchPage =
        window.location.pathname.includes('/videos/watch/') || window.location.pathname.startsWith('/w/');
      if (!isWatchPage) {
            ...
            fetch(fUrl)
              ...
              .then(function (res) {
                ...
                cList.forEach(function (c) {
                  ...
                  card.setAttribute(
                    'aria-label',
                    c.author + ' isimli kullan\u0131c\u0131n\u0131n yorumu: ' + c.content
                  );
                  card.style.cssText =
    ...
    var isInsideYakTubeApp =
      (navigator.userAgent && navigator.userAgent.indexOf('YakTubeMobileApp') !== -1) ||
